...
describe("issue-68 - nested relations not being included", () => {
  class User {
    constructor(
      public id: string,
      public organisationMembers: OrganisationMember[]
    ) {}
  }
  ...
  const OrganisationMemberSerializer = new Serializer<OrganisationMember>(
    "organisation-member"
  );
  ...
  const OrganisationMemberToOrganisationRelator = new Relator<
    OrganisationMember,
    Organisation
  >(
    async (organisationMember: OrganisationMember) =>
  ...
  OrganisationMemberSerializer.setRelators(
    OrganisationMemberToOrganisationRelator
  );

  const org1 = new Organisation(
    "808a1c9b-4b3e-47a2-830a-ff3cdee42d6d",
    "Some Company 1"
  );
  const org2 = new Organisation(
    "a0d14ebd-e9ae-4ec2-b6e8-d7da29c0391e",
    "Some Company 3"
  );
  const org3 = new Organisation(
    "6896c877-2b8e-4349-97b6-364526be98fa",
    "Some Company 2"
  );
  ...
  it("should include all nested relations when included", async () => {
    ...
    expect(
      serialized.included?.filter((i) => i.type === "organisation")
    ).toHaveLength(3);
  });
});
