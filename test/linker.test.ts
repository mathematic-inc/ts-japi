...
describe("Linker Tests", () => {
  ...
  describe("One Argument Linker Tests", () => {
    ...
    it("should construct a One Argument Linker", () => {
      expect(
        () => (UserLinker = new Linker((user) => pathTo(`/users/${user.id}`)))
      ).not.toThrow();
    });
    it.each(
      sliceRandom(User.storage, NUMBER_OF_TESTS).map((user) => user.id)
    )("tests a One Argument Linker on User ID %s", (userId) => {
      // Get dummy data
  ...
  describe("Two Argument Linker Tests", () => {
    describe("With Minimum Options", () => {
      ...
      it.each(
        sliceRandom(User.storage, NUMBER_OF_TESTS).map((user) => user.id)
      )("tests a Two Argument Linker on User ID %s", (userId) => {
        const user = User.find(userId);
        const articles = user.getArticles();

        let link: Link;
        expect(
          () => (link = UserArticlesLinker.link(user, articles))
        ).not.toThrow();
        expect(link).toBeInstanceOf(Link);
    ...
    describe("With All Options", () => {
      ...
      )("tests a Two Argument Linker on User ID %s", (articleId) => {
        const article = Article.find(articleId);
        const user = article.getAuthor();

        let link: Link;
        expect(
          () => (link = UserArticlesLinker.link(user, article))
        ).not.toThrow();
        expect(link).toBeInstanceOf(Link);
