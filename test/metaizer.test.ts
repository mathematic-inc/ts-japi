...
describe("Metaizer Tests", () => {
  ...
  it("should construct several Metaizers", () => {
    ...
    expect(
      () =>
        (UserMetaizer = new Metaizer((user: User) => ({
          createdAt: user.createdAt,
        })))
    ).not.toThrow();
    expect(
      () =>
        (UserArticleMetaizer = new Metaizer(
  ...
  it.each(
    sliceRandom(User.storage, 5).map((user) => user.id)
  )("tests Metaizers on User ID %s", (userId) => {
    const user = User.find(userId);
    const articles = user.getArticles();
    const article = articles[0];

    const metas = new Map<Metaizer<any>, Meta>();
    expect(() =>
      metas.set(UserMetaizer, UserMetaizer.metaize(user))
    ).not.toThrow();
    ...
    expect(() =>
      metas.set(
        UserArticleMetaizer,
        UserArticleMetaizer.metaize(user, articles)
      )
    ).not.toThrow();
    ...
    if (article) {
      expect(() =>
        metas.set(
          UserArticleMetaizer,
          UserArticleMetaizer.metaize(user, article)
        )
      ).not.toThrow();
      expect(metas.get(UserArticleMetaizer)).toBeInstanceOf(Meta);
      expect(getJSON(metas.get(UserArticleMetaizer))).toEqual({
